...
export default function ClientesPage() {
  ...
  // Email y Acción (editar email) son exclusivos de Administrador — el
  // backend (PUT /clientes/:id) ya solo lo permite a ese rol, así que un
  // Gerente viendo esos campos no podría hacer nada con ellos.
  const puedeVerEmail = rolSatisface(usuario?.rol, ['Administrador']);
  ...
  const filtrados = clientes.filter((c) => {
    ...
    return (
      ...
      (puedeVerEmail && c.email && c.email.toLowerCase().includes(q))
    );
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder={puedeVerEmail ? 'Buscar por nombre, DNI o email…' : 'Buscar por nombre o DNI…'}
          value={busqueda}
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <table className="min-w-full divide-y divide-gray-100">
          ...
          <tbody className="divide-y divide-gray-100">
            {cargando ? (
              <tr>
                <td colSpan={puedeVerEmail ? 5 : 3} className="py-10 text-center text-sm text-gray-400">
                  Cargando…
                </td>
              </tr>
            ) : filtrados.length === 0 ? (
